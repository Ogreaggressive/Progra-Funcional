...
(test (run '{rec {sum {fun {n} {if-tf {zero?? n} 0 {+ n {sum {- n 1}}}}}} {sum 5}}) 15)

(test (run '{with {addN {fun {n} {fun {x} {+ x n}}}}
            {{addN 7} 10}}) 17)

;no da con (= n 1) por lo que idee tener un zero?? (-n 1)
(test (run '{rec {fib {fun {n}
                    {if-tf {zero?? n}
                      0
                      {if-tf {zero?? {- n 1}}
                        1
